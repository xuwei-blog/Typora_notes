...
```assembly
# 压栈（此时假设SS:SP = 1000:000E = 1000E）（段地址：偏移地址 = 物理地址）
mov ax = 0123
push ax
# >> 压栈后 SS:SP = 1000:000C , 占用2个字节空间
```
...
![image-20240604161136751](https://typora-notes-codervv.oss-cn-shanghai.aliyuncs.com/img_for_typora/202406041611866.png)

```assembly
# 用debug在 2000:1F60内存地址写数据
-e 2000:1F60
2000:1F60 00.12 00.34 00.56 00.78 00.9A
# 查看内存地址，都可以看到同一片内存地址
-d 2000:1F60
-d 2100:0F60
```
...
>
> 8086是小端存储，数据高位AH（34） 放在高地址(073F:0001) , 数据低位AL（12）放在低地址（073F:0000）

![image-20240604163605575](https://typora-notes-codervv.oss-cn-shanghai.aliyuncs.com/img_for_typora/202406041636646.png)



### flag寄存器

![image-20240628163600027](https://typora-notes-codervv.oss-cn-shanghai.aliyuncs.com/img_for_typora/202406281636154.png)

| 标志位 | 含义                  | 第一种情况 DOS下表示          | 第二种情况 DOS下表示            |
| ------ | --------------------- | ----------------------------- | ------------------------------- |
| ZF     | 零标志位              | ZF = 1  ,  表示运算为0  ,  ZR | ZF = 0  ,  表示运算不为0  ,  NZ |
| PF     | 奇偶标志位,是否为偶数 | PF = 1  ,  表示为偶数  ，PE   | PF = 0  ,  表示为奇数  ，PO     |
